...
def create_contador_in_production():
    ...
    try:
        ...
        # 2. PASO 2: Crear el nuevo usuario contador para EstudioCastillo
        print("\n⚙️ Enviando solicitud para crear el nuevo usuario contador para EstudioCastillo...")
        
        contador_payload = json.dumps({
            "email": "login@estudiocastillo.com",
            "password": "Ecastillo2026!",
            "nombre": "Estudio",
            "apellido": "Castillo",
            "dni": "99887755",  # DNI diferente al anterior para evitar colisiones
            "telefono": "1122334455",
            "rol": "CONTADOR",
            "activo": True
        })
        ...
        res_user = conn.getcall = conn.getresponse()
        ...
        if res_user.status in (200, 201):
            print("\n🎉 ¡Nuevo Usuario Contador para EstudioCastillo Creado Exitosamente en Producción! 🎉")
            print("   Email: login@estudiocastillo.com")
            print("   Contraseña: Ecastillo2026!")
            print("   Rol: CONTADOR")
            ...
            print("   Intentando actualizar el rol del usuario existente a CONTADOR y actualizar sus credenciales...")
            
            # Busquemos los usuarios existentes para ver si podemos actualizar el usuario
            conn.request("GET", "/api/users", headers=contador_headers)
            res_list = conn.getcall = conn.getcall = conn.getresponse()
            data_list = res_list.read()
            
            if res_list.status == 200:
                users = json.loads(data_list.decode('utf-8'))
                target_user = next((u for u in users if u.get("email") == "login@estudiocastillo.com"), None)
                
                if target_user:
                    user_id = target_user.get("id")
                    print(f"👤 Usuario encontrado en producción con ID: {user_id}. Asegurando rol a CONTADOR...")
                    ...
                    if res_update.status == 200:
                        print("🎉 ¡Usuario actualizado con éxito en producción! 🎉")
                    else:
                        print("❌ Falló la actualización del usuario.")
                else:
                    print("❌ No se encontró el usuario 'login@estudiocastillo.com' en la lista de producción.")
            else:
