...
@router.post("/sectors")
async def create_sector(data: SectorCreate, current_user=Depends(get_current_user)):
    item = await prisma.sector.create(data={"nombre": data.nombre})
    return item

@router.delete("/sectors/{id}")
async def delete_sector(id: str, current_user=Depends(get_current_user)):
...
async def create_puesto(data: PuestoCreate, current_user=Depends(get_current_user)):
    item = await prisma.puesto.create(data={"nombre": data.nombre, "sectorId": data.sectorId})
    return item

@router.delete("/job-positions/{id}")
async def delete_puesto(id: str, current_user=Depends(get_current_user)):
...
async def create_localidad(data: LocalidadCreate, current_user=Depends(get_current_user)):
    item = await prisma.localidad.create(data={"nombre": data.nombre, "provincia": data.provincia})
    return item

@router.delete("/service-locations/{id}")
async def delete_localidad(id: str, current_user=Depends(get_current_user)):
...
async def create_area(data: AreaCreate, current_user=Depends(get_current_user)):
    item = await prisma.areaoperativa.create(data={"nombre": data.nombre})
    return item

@router.delete("/operational-areas/{id}")
async def delete_area(id: str, current_user=Depends(get_current_user)):
...
# ========== USERS ADMIN ==========
@router.get("/users")
async def list_users_admin(current_user=Depends(get_current_user)):
