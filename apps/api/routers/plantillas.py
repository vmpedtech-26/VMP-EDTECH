...
@router.get("")
async def list_plantillas(current_user=Depends(require_admin)):
...
@router.post("")
async def create_plantilla(data: PlantillaCreate, current_user=Depends(require_admin)):
...
@router.delete("/{id}")
async def delete_plantilla(id: str, current_user=Depends(require_admin)):
