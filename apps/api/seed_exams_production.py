...
def main():
    try:
        ...
        cur = conn.cursor()
        print("🌱 Iniciando carga de contenido educativo en PRODUCCIÓN (Direct SQL)...")
        ...
        if not curso_map:
            print("❌ No se encontraron cursos en la base de datos. Abortando.")
            return

        # 2. Limpiar contenido previo para evitar duplicados (opcional, pero útil para re-seed)
        # cur.execute("DELETE FROM preguntas; DELETE FROM modulos;")

        # --- CURSO: Manejo Defensivo Livianos (MDL-001) ---
        if "MDL-001" in curso_map:
            curso_id = curso_map["MDL-001"]
            print(f"  - Preparando contenido para MDL-001 (ID: {curso_id})")
        ...
        if "MDP-001" in curso_map:
            ...
            print(f"  - Preparando contenido para MDP-001 (ID: {curso_id})")
        ...
        if "PA-001" in curso_map:
            ...
            print(f"  - Preparando contenido para PA-001 (ID: {curso_id})")
        ...
        conn.commit()
        print("✅ Carga finalizada con éxito en PRODUCCIÓN.")
        cur.close()
        conn.close()
