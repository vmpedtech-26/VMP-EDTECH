...
def build_student_pdf(student):
    ...
    # Top Verificado Badge Pill
    c.setFillColor(navy)
    c.roundRect(40, 290, 95, 18, 9, fill=True, stroke=False)
    c.setFillColor(cyan)
    c.setFont('Helvetica-Bold', 8)
    c.drawString(50, 295, '✓ VERIFICADO')

    # Student Photo Insertion
    if os.path.exists(student['photo_path']):
        c.drawImage(student['photo_path'], 38, 75, width=149, height=205, preserveAspectRatio=False)

    # Bottom Active Status Pill
    c.setFillColor(navy)
    c.roundRect(38, 50, 149, 20, 4, fill=True, stroke=False)
    c.setFillColor(HexColor('#FFFFFF'))
    c.setFont('Helvetica-Bold', 7.5)
    c.drawCentredString(112, 56, 'ESTADO: ACTIVO & ACREDITADO')
    ...
    c.drawCentredString(297, 375, 'Esta credencial certifica la aprobación de la capacitación oficial:')
    c.setFillColor(cyan)
    c.setFont('Helvetica-Bold', 14)
    c.drawCentredString(297, 352, 'Conducción Segura: Flota Liviana')
    ...
    # Center QR Container
    c.setFillColor(navy)
    c.roundRect(205, 80, 185, 165, 10, fill=True, stroke=False)
    ...
    c.drawImage(ImageReader(buf), 238, 122, width=118, height=118)

    c.setFillColor(cyan)
    c.setFont('Helvetica-Bold', 8)
    c.drawCentredString(297, 108, 'SCAN TO VERIFY  |  VMP')
    c.setFillColor(HexColor('#FFFFFF'))
    c.setFont('Helvetica-Bold', 9.5)
    c.drawCentredString(297, 92, f"CÓDIGO: {student['code_display_short']}")

    # Bottom Logos (extracted from Coivalsa template)
    if os.path.exists('scratch/logo_p2_0.png'):
        c.drawImage('scratch/logo_p2_0.png', 30, 30, width=535, height=45, preserveAspectRatio=True)

    # Instructor Text
    c.setFillColor(text_muted)
    c.setFont('Helvetica', 8.5)
    c.drawCentredString(297, 55, 'Acreditado por: Pedro Orejas - Instructor VMP | Mat. N° 2206823')
    ...
    print(f"✅ Generated Executive 2-Page Vector PDF: {pdf_path}")
...
for st in students_data:
    pdf_files.append(build_student_pdf(st))
...
print(f"🎉 Created Executive Master Combined PDF (8 pages): {master_pdf_path}")
