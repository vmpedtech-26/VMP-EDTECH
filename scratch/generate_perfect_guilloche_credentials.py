import os, sys, qrcode, fitz, math
...
def draw_guilloche_watermark(c, w=595, h=400):
    c.saveState()
...
def build_student_pdf_perfect(student):
    ...
    # Guilloché Security Watermark Pattern
    draw_guilloche_watermark(c, 595, 400)
    ...
    c.drawCentredString(297, 114, 'SCAN TO VERIFY  |  VMP')
    c.setFillColor(HexColor('#FFFFFF'))
    c.setFont('Helvetica-Bold', 9)
...
for st in students_data:
    pdf_files.append(build_student_pdf_perfect(st))
